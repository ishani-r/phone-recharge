...
class ContactDatatable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ->eloquent($query)
            // ->addColumn('action', 'contactdatatable.action')
            ->addColumn('action', function($data){
                $result = '<div class="btn-group">';
                $result .= '<a href="'.route('admin.replay',$data->id).'"><button class="btn-sm btn-outline-warning" style="border-radius: 2.1875rem;">Replay</button></a>';
                return $result;
            })
            ->addIndexColumn();
    }
     ...
     *
     * @param \App\Models\ContactDatatable $model
